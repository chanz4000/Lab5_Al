...
public class Puzzle {
	...
	public int computeH2(Node currentState) {
		int output = 0;
		for (int title = 0; title < MAX_ROW*MAX_COL; title++) {
			int[] currentPos = currentState.getLocation(title);
			int []goalPos= goalState.getLocation(title);
			output+= PuzzleUtils.manhattanDistance(currentPos, goalPos);
		}
	...
	public int computeH1(Node currentState) {
		...
		for (int i = 0; i < MAX_ROW; i++) {
			for (int j = 0; j < MAX_COL; j++) {
				int titleCurrent =currentState.getTile(i, j);
				int titleGoal = goalState.getTile(i, j);
				if(titleCurrent!=0 && titleCurrent!=titleGoal) {
					result++;
				}
			}
		}
